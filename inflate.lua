...
--         x = bitOp.bxor(c,w)
--         y = CRC32[x]
--         z = rshift(crc&0xFFFFFFFF,8)

--         crc = bitOp.bxor(y, z)

-- --        crc = bitOp.bxor(CRC32[bitOp.bxor(c, band(crc, 255))], rshift(crc, 8))
--     end
--     crc = bitOp.bnot(crc)&0xFFFFFFFF
--     if crc < 0 then
--         crc = crc + 4294967296
--     end
--     return crc
...
--             return packed and self:inflate(offset, verify and crc or nil) or string.sub(self.buffer, offset, offset + size - 1)
--         end
--     end
